...
namespace GMIS.FileServices
{
    public class FileService : IFileService
    {
        ...
        {
            var date = "_" + DateTime.Now.ToString("yyyyMMddHHmmss");
            IFormFile myFile = file;
            var fileExtension = Path.GetExtension(myFile.FileName);
            var fileName = Path.GetFileNameWithoutExtension(myFile.FileName);
            string rootpath = _environment.WebRootPath;
            string folderPath = Path.Combine(rootpath, "Uploads");
            if (!Directory.Exists(folderPath))
                Directory.CreateDirectory(folderPath);
            var tempFolderName = Path.GetTempFileName();
            string physicalfilepath = folderPath + "\\file" + date + fileExtension;
            var relativePath = Path.Combine("Uploads", "file" + date + fileExtension);
